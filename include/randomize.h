...
void randomizeMap(const std::filesystem::path romfsDir, std::filesystem::path targetDir, const std::vector<TrackedFile> &filesToEdit, const GameObjTracker trackers[], const int nTrackers, WeaponClass &weaponTypes, WeaponLists &weaponLists, const bool chaos = false, const bool debug = false);
